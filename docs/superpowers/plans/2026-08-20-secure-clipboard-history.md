...
internal sealed interface ClipboardGatewayResult {
    /** TimestampUnavailable is explicit unknown evidence; this value is never null. */
...
internal interface ClipboardActivationStore {
    ...
    fun readPrimarySuppression(): ClipboardSuppressionState
    fun writePrimarySuppression(state: ClipboardSuppressionState): Boolean
}

internal sealed interface ClipboardSuppressionState {
    data object NotSuppressed : ClipboardSuppressionState
    class Suppressed(val sourceMarker: ClipboardSourceMarker) : ClipboardSuppressionState
}
...
`clearAllAndResume()` écrit d’abord `CLEARING_ENABLED`, retire le listener, ferme/annule le worker et le joint, capture le primary clip courant puis persiste `Suppressed(result.sourceMarker)` avant d’appeler `repository.clearAll()`; seulement après la purge il écrit `ENABLED`, recrée queue/permits/worker et rattache le listener, sans réimporter le primary clip ancien. Il persiste donc aussi la preuve non nulle d’un clip rejeté/borné ou absent ; si la capture ne fournit pas de timestamp API 31+, `TimestampUnavailable` reste fail-closed. La suppression survit au redémarrage et seul `DEFINITELY_CHANGED` dans un callback listener écrit `NotSuppressed`. `disableAndPurge` suit la même barrière puis purge et désactive, mais permet un futur `enable` qui recrée queue+worker. Les générations ne filtrent que `state/events`, jamais le consumer.

`ClipboardActivationStore` persiste l’enum et le `ClipboardSuppressionState` par `SharedPreferences.commit()` et vérifie le booléen de retour. `disableAndPurge` écrit `DISABLING` **avant** de retirer listener/queue/grants et d’effacer ; le startup voyant cet état reprend la purge sans aucune capture, écrit `NotSuppressed`, puis écrit `DISABLED` en dernier. `resetProtectedHistory` suit `RESETTING_ENABLED`, arrête capture/grants, capture/persiste `Suppressed(result.sourceMarker)` avant purge, efface DB/containers/caches, supprime puis recrée les clés, revient `ENABLED` et rattache un listener vide sans réimporter l’ancien primary clip. Aucun contenu n’est effacé pour `KEY_UNAVAILABLE` tant que l’utilisateur n’a pas confirmé cette action.
...
`start()` branche exhaustivement sur l’état durable : `DISABLED` ne crée rien ; `CLEARING_ENABLED` reprend le clear puis revient `ENABLED` sans import ; `DISABLING` reprend la purge ; `RESETTING_ENABLED` reprend le reset ; `ENABLED` réconcilie, crée FIFO/worker/listener puis importe une fois le primary clip uniquement si `readPrimarySuppression()` vaut `NotSuppressed`. Il ne compare ni ne change jamais cette suppression. `enable()` est une nouvelle décision explicite : il écrit d’abord `NotSuppressed`, puis `ENABLED`, crée les mêmes ressources et importe le clip courant une fois ; si une écriture durable échoue, il publie un Failure sûr et n’attache pas le listener.
